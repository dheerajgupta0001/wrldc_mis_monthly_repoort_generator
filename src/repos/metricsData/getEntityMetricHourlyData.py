...
def getEntityMetricHourlyData(appDbConnStr: str, entityName: str, metricName: str, startDt: dt.datetime, endDt: dt.datetime) -> List[IMetricsDataRecord]:
    targetColumns = ['TIME_STAMP', 'ENTITY_TAG', 'METRIC_NAME', 'DATA_VALUE']

    metricsFetchSql = """
            select {0}
            from mis_warehouse.state_files_data sf
            where 
            sf.ENTITY_TAG = :1
