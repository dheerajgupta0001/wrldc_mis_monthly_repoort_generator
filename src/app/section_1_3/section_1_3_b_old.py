...
def fetchSection1_3_bContext(appDbConnStr: str, startDt: dt.datetime, endDt: dt.datetime) -> ISection_1_3_b:
    ...
    metricValueRename = {
        ...
        'so_far_highest_req_time': 'so_far_highest_requirement_dt',
        'so_far_highest_avail_time': 'so_far_highest_availability_dt'
    }
    constConfig = {}
    ...
    allEntityPeakloadSheddingMWVals = mRepo.getAllEntityMaxMetricDataFromHourly(
        metricNamesdb['loadShedding'], startDt, endDt
    )
    allEntityPeakAvailMWVals = mRepo.getAllEntityMaxMetricDataFromHourly(
        metricNamesdb['peakAvailability'], startDt, endDt)

    allEntityPeakloadSheddingMWDf = pd.DataFrame(
        allEntityPeakloadSheddingMWVals)
    allEntityPeakAvailMWDf = pd.DataFrame(allEntityPeakAvailMWVals)

    allEntityPeakReqMWDf = allEntityPeakAvailMWDf
    allEntityPeakReqMWDf['data_value'] = allEntityPeakAvailMWDf['data_value'] + \
        allEntityPeakloadSheddingMWDf['data_value']
    ...
    soFarHighestAllEntityAvailMWDf = pd.DataFrame(
        soFarHighestAllEntityAvailMWVals)
    ...
    for rIter in range(dataRecords.shape[0]):
        ...
        if monthPeakReq > soFarHighestReq:
            row['shortage_Y'] = calcShortagePerc(
                row['shortage_Y'], soFarHighestReq, soFarHighestAvail)
        ...
        mRepo.insertSoFarHighest(
            constituent, metricName, startDt, monthPeakReq, monthPeakReqTime)

        metricName = metricValueRename['so_far_highest_avail_time']
        mRepo.insertSoFarHighest(
            constituent, metricName, startDt, monthPeakAvail, monthPeakAvailTime)

    power_req_so_far_hgst: List[ISection_1_3_b["power_req_and_avail_and_so_far_highest"]] = [
    ]

    for i in dataRecords.index:
        highestReqTime = dataRecords[metricValueRename['so_far_highest_req_time']].loc[i]
        highestAvailTime = dataRecords[metricValueRename['so_far_highest_avail_time']].loc[i]
        ...
        powr_so_far_single: ISoFarHighestDataRow = {
    ...
    secData: ISection_1_3_b = {"so_far_hig_req_avail": power_req_so_far_hgst}
    return secData


def calcShortagePerc(shortage_Y, req, avail):
